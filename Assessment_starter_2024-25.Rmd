...
paid_quantity <- data_march2024 %>% 
  ...
  summarise(total = sum(number_of_paid_items))

total10 <- paid_quantity %>% 
  top_n(10) %>% 
  arrange(desc(total))
...
health_board_2024 <- read_csv("https://www.opendata.nhs.scot/dataset/9f942fdb-e59e-44f5-b534-d6e17229cc7b/resource/652ff726-e676-4a20-abda-435b98dd7bdc/download/hb14_hb19.csv") %>% 
  clean_names()

hb24 <-  health_board_2024 %>% 
  select("hb", "hb_name") %>% 
  rename( "hbt" = "hb")
  
joineddata <- hb24 %>% 
  full_join(data_march2024)
...
```{r}
filtered <- joineddata %>%
  replace_na(list(hb_name = 'Scottish Ambulane Service'))
...
paracetamol_data <- joineddata2 %>% 
  ...
  group_by(hb_name) %>% 
  summarise(paraceportion = sum(paid_quantity)/ mean(hb_population))
...
prescriptions2020 <- read_csv("https://www.opendata.nhs.scot/dataset/84393984-14e9-4b0d-a797-b288db64d088/resource/9581bd8d-5568-4462-93a6-6d7bfbbe7cbc/download/pitc202003.csv") %>% 
  clean_names()
